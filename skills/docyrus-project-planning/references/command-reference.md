...
Initialise a new project plan. Creates the plan graph, all eight standard Docyrus phases (Architecture → Deployment), `FEATURES.mermaid`, and `DATA_SOURCES.mermaid` stubs in one shot. Safe to call on an already-initialised project — prints a notice and exits without modifying anything.

```bash
docyrus project-plan init
```

Standard phases created (in order): Architecture, Data Model, Automations, AI Agents, Templates, UI Components, Testing, Deployment.

Returns: `{ alreadyInitialized, graphPath, markdownPath, featuresPath, dataSourcesPath, phaseCount }`
...
Returns paths for: `projectPlanDir`, `graphPath`, `markdownPath`, `featuresMermaidPath`, `dataSourcesMermaidPath`.
...
Write or update the Mermaid diagram files.

```bash
# Regenerate FEATURES.mermaid; create DATA_SOURCES.mermaid if it doesn't exist
docyrus project-plan render-diagrams

# Also overwrite DATA_SOURCES.mermaid (loses manual edits)
docyrus project-plan render-diagrams --forceDataSources true
```

| Flag | Description |
|------|-------------|
| `--forceDataSources` | Overwrite `DATA_SOURCES.mermaid` even if it already exists |

Returns: `{ featuresPath, dataSourcesPath, featuresUpdated, dataSourcesUpdated }`

`FEATURES.mermaid` is always regenerated from the live graph. `DATA_SOURCES.mermaid` is only written when the file is new (or `--forceDataSources` is set) — manual edits are preserved by default.
