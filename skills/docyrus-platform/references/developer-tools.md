...
- Agent runtime & dev tooling: platform AI chat (`docy`), pi Cowork/Coding agents (`worky`/`cody`/`coder`), agent bridge server (`server`), browser automation (`browser`), repo knowledge graph (`knowledge`), project plan graph (`project-plan`), and release management (`release`)
