...
>
> **Not sure how to write the SQL?** `docyrus dsql generate "<question>"` turns a natural-language question into a DSQL query via the DSQL generator agent, and `docyrus dsql ask "<question>"` generates it and runs it in one step. The agent follows every rule in this document.
