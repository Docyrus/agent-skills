...
- **Placeholders are Handlebars `{{field}}`** over the **expanded record** at send time: bare fields `{{slug}}`, built-ins `{{name}}`/`{{autonumber_id}}`, expanded enum/relation/user fields as objects `{{stage.name}}`, and **child data source collections** via `{{#each child.{from}__{using}}}` (auto-fetched from the body — see [references/templating.md](references/templating.md#child-data-source-collections)). Not JSONata, not `${}`.
